...
void Java_com_jokes_utils_AudioEncoder_initEncoder(JNIEnv *env,
...
void Java_com_jokes_utils_AudioEncoder_destroyEncoder(
		JNIEnv *env, jobject jobj) {
	int res = lame_close(lame);
	LOGD("Deinit returned: %d", res);
}

void Java_com_jokes_utils_AudioEncoder_encodeFile(JNIEnv *env,
	...
	LOGD("Source path %s", source_path);
	LOGD("Target path %s", target_path);
